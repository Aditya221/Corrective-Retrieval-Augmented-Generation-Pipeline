"""
Query Rewriter Benchmark

Tests both rewrite strategies (expand/pivot) against known failure cases.
Measures latency and validates that rewritten queries are meaningfully
different from the originals.

Run:
    python bench_rewriter.py
"""
...
# Test cases mirror the evaluator failures:
# Each entry is (original_query, failed_chunks, trigger_grade, what_we_expect)
REWRITE_TESTS = [
    {
...
def run_rewriter_benchmark():
    print("\n" + "="*60)
    print("  QUERY REWRITER BENCHMARK")
    print("="*60)
    ...
    for i, test in enumerate(REWRITE_TESTS):
        ...
        # Basic validation: rewritten query should differ from original
        ...
        print(f"  Different : [{different_mark}] {'Yes' if is_different else 'No — PROBLEM'}")
    ...
    print(f"    Query rewrite     : ~{avg_latency:.0f}ms  (LLM)")
    print(f"    Re-retrieval      :   ~0.5ms")
    print(f"    Re-evaluation     :  ~20.0ms")
    print(f"    {'─'*30}")
    print(f"    Total (no answer) : ~{0.5+20+avg_latency+0.5+20:.0f}ms  (before final LLM)")
    print(f"{'='*60}")
